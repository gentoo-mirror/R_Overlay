# Copyright 1999-2023 Gentoo Foundation
# Distributed under the terms of the GNU General Public License v2

EAPI=8

inherit R-packages

DESCRIPTION='Tools for Single Cell Genomics'
KEYWORDS="~amd64 ~x64-macos ~arm64-macos"
SRC_URI="http://cran.r-project.org/src/contrib/Seurat_4.4.0.tar.gz"
LICENSE='MIT'

IUSE="${IUSE-} r_suggests_ape r_suggests_biobase r_suggests_biocgenerics
	r_suggests_data_table r_suggests_enrichr r_suggests_ggrastr
	r_suggests_hdf5r r_suggests_iranges r_suggests_limma r_suggests_metap
	r_suggests_mixtools r_suggests_r_utils r_suggests_rfast2
	r_suggests_rsvd r_suggests_s4vectors r_suggests_testthat
	r_suggests_vgam"
R_SUGGESTS="
	r_suggests_ape? ( sci-CRAN/ape )
	r_suggests_biobase? ( sci-BIOC/Biobase )
	r_suggests_biocgenerics? ( sci-BIOC/BiocGenerics )
	r_suggests_data_table? ( sci-CRAN/data_table )
	r_suggests_enrichr? ( sci-CRAN/enrichR )
	r_suggests_ggrastr? ( sci-CRAN/ggrastr )
	r_suggests_hdf5r? ( sci-CRAN/hdf5r )
	r_suggests_iranges? ( sci-BIOC/IRanges )
	r_suggests_limma? ( sci-BIOC/limma )
	r_suggests_metap? ( sci-CRAN/metap )
	r_suggests_mixtools? ( sci-CRAN/mixtools )
	r_suggests_r_utils? ( sci-CRAN/R_utils )
	r_suggests_rfast2? ( sci-CRAN/Rfast2 )
	r_suggests_rsvd? ( sci-CRAN/rsvd )
	r_suggests_s4vectors? ( sci-BIOC/S4Vectors )
	r_suggests_testthat? ( sci-CRAN/testthat )
	r_suggests_vgam? ( sci-CRAN/VGAM )
"
DEPEND="sci-CRAN/future
	sci-CRAN/png
	>=sci-CRAN/Rcpp-1.0.7
	sci-CRAN/cowplot
	virtual/Matrix
	sci-CRAN/miniUI
	sci-CRAN/reticulate
	sci-CRAN/Rtsne
	>=sci-CRAN/scattermore-1.2
	sci-CRAN/future_apply
	sci-CRAN/igraph
	sci-CRAN/spatstat_explore
	sci-CRAN/tibble
	sci-CRAN/scales
	sci-CRAN/RANN
	sci-CRAN/ica
	sci-CRAN/irlba
	>=sci-CRAN/leiden-0.3.1
	virtual/Matrix
	sci-CRAN/shiny
	>=sci-CRAN/ggplot2-3.3.0
	virtual/KernSmooth
	sci-CRAN/lmtest
	>=sci-CRAN/plotly-4.9.0
	sci-CRAN/progressr
	>=sci-CRAN/SeuratObject-4.1.4
	virtual/cluster
	sci-CRAN/jsonlite
	virtual/MASS
	sci-CRAN/pbapply
	sci-CRAN/spatstat_geom
	>=sci-CRAN/uwot-0.1.14
	>=dev-lang/R-4.0.0
	sci-CRAN/ggrepel
	sci-CRAN/ggridges
	>=sci-CRAN/RcppAnnoy-0.0.18
	sci-CRAN/rlang
	sci-CRAN/fitdistrplus
	sci-CRAN/httr
	sci-CRAN/patchwork
	sci-CRAN/purrr
	sci-CRAN/RColorBrewer
	sci-CRAN/ROCR
	>=sci-CRAN/sctransform-0.4.0
"
RDEPEND="${DEPEND-}
	>=sci-CRAN/Rcpp-0.11.0
	sci-CRAN/RcppProgress
	sci-CRAN/RcppEigen
	${R_SUGGESTS-}
"

_UNRESOLVED_PACKAGES=(
	'DESeq2'
	'GenomeInfoDb'
	'GenomicRanges'
	'MAST'
	'monocle'
	'rtracklayer'
	'SingleCellExperiment'
	'SummarizedExperiment'
)
