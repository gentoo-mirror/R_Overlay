# Copyright 1999-2024 Gentoo Foundation
# Distributed under the terms of the GNU General Public License v2

EAPI=8

inherit R-packages

DESCRIPTION='Tidy Finance Helper Functions'
KEYWORDS="~amd64 ~x64-macos ~arm64-macos"
SRC_URI="http://cran.r-project.org/src/contrib/tidyfinance_0.1.0.tar.gz"
LICENSE='MIT'

IUSE="${IUSE-} r_suggests_dbi r_suggests_dbplyr r_suggests_frenchdata
	r_suggests_readxl r_suggests_rpostgres r_suggests_testthat"
R_SUGGESTS="
	r_suggests_dbi? ( >=sci-CRAN/DBI-1.1.0 )
	r_suggests_dbplyr? ( >=sci-CRAN/dbplyr-2.3.2 )
	r_suggests_frenchdata? ( >=sci-CRAN/frenchdata-0.2.0 )
	r_suggests_readxl? ( >=sci-CRAN/readxl-1.4.2 )
	r_suggests_rpostgres? ( >=sci-CRAN/RPostgres-1.4.5 )
	r_suggests_testthat? ( >=sci-CRAN/testthat-3.0.0 )
"
DEPEND=">=dev-lang/R-4.1
	>=sci-CRAN/tibble-3.2.1
	>=sci-CRAN/dplyr-1.1.4
	>=sci-CRAN/purrr-1.0.2
	>=sci-CRAN/rlang-1.1.3
	>=sci-CRAN/tidyr-1.3.0
	>=sci-CRAN/lubridate-1.9.3
	sci-CRAN/lifecycle
"
RDEPEND="${DEPEND-} ${R_SUGGESTS-}"
