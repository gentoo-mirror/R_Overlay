# Copyright 1999-2016 Gentoo Foundation
# Distributed under the terms of the GNU General Public License v2
# $Header: $

EAPI=5

inherit R-packages

DESCRIPTION='Extension to ggplot2'
SRC_URI="http://cran.r-project.org/src/contrib/GGally_1.0.1.tar.gz"
LICENSE='GPL-2+'

IUSE="${IUSE-} r_suggests_arm r_suggests_geosphere r_suggests_ggmap
	r_suggests_igraph r_suggests_intergraph r_suggests_maps
	r_suggests_network r_suggests_rcolorbrewer r_suggests_scagnostics
	r_suggests_scales r_suggests_sna r_suggests_survival
	r_suggests_testthat r_suggests_tnet"
R_SUGGESTS="
	r_suggests_arm? ( >=sci-CRAN/arm-1.7 )
	r_suggests_geosphere? ( >=sci-CRAN/geosphere-1.3.11 )
	r_suggests_ggmap? ( >=sci-CRAN/ggmap-2.3 )
	r_suggests_igraph? ( >=sci-CRAN/igraph-1.0.0 )
	r_suggests_intergraph? ( >=sci-CRAN/intergraph-2.0.0 )
	r_suggests_maps? ( >=sci-CRAN/maps-2.3.9 )
	r_suggests_network? ( >=sci-CRAN/network-1.7.2 )
	r_suggests_rcolorbrewer? ( >=sci-CRAN/RColorBrewer-1.0.5 )
	r_suggests_scagnostics? ( >=sci-CRAN/scagnostics-0.2.4 )
	r_suggests_scales? ( >=sci-CRAN/scales-0.2.3 )
	r_suggests_sna? ( >=sci-CRAN/sna-2.3.1 )
	r_suggests_survival? ( >=sci-CRAN/survival-2.37.4 )
	r_suggests_testthat? ( sci-CRAN/testthat )
	r_suggests_tnet? ( >=sci-CRAN/tnet-3.0 )
"
DEPEND=">=dev-lang/R-2.14
	>=sci-CRAN/ggplot2-2.0.0
	>=sci-CRAN/gtable-0.1.2
	>=sci-CRAN/plyr-1.8
	>=sci-CRAN/reshape-0.8.4
"
RDEPEND="${DEPEND-} ${R_SUGGESTS-}"

_UNRESOLVED_PACKAGES=(
	'>=sci-CRAN/knitr-1.12'
	'>=sci-CRAN/rmarkdown-0.9.2'
	'>=sci-CRAN/roxygen2-5.0.0'
)
